...
echo $results_array >results.txt
for result in "${results_array[@]}"; do
    ...
    printf "Case %s: Expected \$%.2f, Got \$%.2f, Error \$%.2f (%s days, %s miles, \$%s receipts)\n" \
        "$case_num" "$expected" "$actual" "$error" "$trip_duration" "$miles_traveled" "$receipts_amount"
done
